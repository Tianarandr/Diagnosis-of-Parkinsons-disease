...
#/Users/tahirintsoa/DATA_PROJET/dl-tsc/archives/UCR_TS_Archive_2015 
...
def save_logs2(output_directory, hist, y_pred, y_true, duration, lr=True):
    hist_df = pd.DataFrame(hist.history)
    hist_df.to_csv(output_directory + 'history.csv', index=False)
...
def transforme_data(name_data):
    count = 0
    # select dataset GaCo / JuCo / SiCo
    name_data_Co =  name_data+'Co'
    for name in os.listdir('/Users/tahirintsoa/DATA_PARKINSON/CSV'):
        if name_data_Co in name:
            if '_01.csv' in name :
                count += 1
    files = []
    for name in os.listdir('/Users/tahirintsoa/DATA_PARKINSON/CSV'):
        if name_data_Co in name:
            if '_01.csv' in name :
                files.append('/Users/tahirintsoa/DATA_PARKINSON/CSV/' + name)
...
# select dataset GaPt / JuPt / SiPt
        count = 0
        name_data_Pt =  name_data+'Pt'

        for name in os.listdir('/Users/tahirintsoa/DATA_PARKINSON/CSV'):
            if name_data_Pt in name:
                if '_01.csv' in name :                
                    count += 1
        files = []
        for name in os.listdir('/Users/tahirintsoa/DATA_PARKINSON/CSV'):
        ...
        name_file = name_data_Co + name_data_Pt
        df2.to_csv('/Users/tahirintsoa/DATA_PARKINSON/Transformed_Data/'+name_file+'/Transformed_Co.csv')
        df4.to_csv('/Users/tahirintsoa/DATA_PARKINSON/Transformed_Data/'+name_file+'/Transformed_Pt.csv')